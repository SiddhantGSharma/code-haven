...
const Vertex Graph::InvalidVertex = {"_CS225INVALIDVERTEX", "", "", 0, 0, 0};
...
Vertex Graph::setVertexLabel(Vertex v, string l)
{
...
bool Graph::assertEdgeExists(Vertex source, Vertex destination, string functionName) const
{
    ...
    {
        if (functionName != "")
            error(functionName + " called on nonexistent edge " + source.name + " -> " + destination.name);
        return false;
    }
    ...
    if(!directed)
    {
        ...
        {
            if (functionName != "")
                error(functionName + " called on nonexistent edge " + destination.name + " -> " + source.name);
            return false;
        }
    }
    return true;
...
void Graph::DFS()
{
    for (Vertex v : getVertices()) {
        // labels each vertex as UNEXPLORED
        setVertexLabel(v, "UNEXPLORED");
    }

    for (Edge e : getEdges()) {
        // labels each edge as UNEXPLORED
        setEdgeLabel(e.source, e.dest, "UNEXPLORED");
    }

    for (Vertex v : getVertices()) {
        if (getVertexLabel(v) == "UNEXPLORED") {   
            DFS(v);
        }
    }
}
...
{
    setVertexLabel(v, "EXPLORED");

    for (Vertex adj : getAdjacent(v)) {
        if (getVertexLabel(adj) == "UNEXPLORED") {
            setEdgeLabel(v, adj, "DISCOVERY");
            DFS(adj);
        } else if (getEdgeLabel(v, adj) == "UNEXPLORED") {
            setEdgeLabel(v, adj, "BACK");
        }
    }
}
